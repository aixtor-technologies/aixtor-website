        ...
        {items.map(({ title: itemTitle, url }, idx) => (
          <li key={`${itemTitle}-${idx}`}>
            <Link
              href={url || "/"}
              title={itemTitle}
              className="relative transition-all duration-500 hover:text-primary after:absolute after:-bottom-0.5 after:left-0 after:h-px after:w-0 after:bg-linear-to-r after:from-primary after:to-secondary after:transition-all after:duration-500 hover:after:w-full"
            >
              {itemTitle}
            </Link>
          </li>
